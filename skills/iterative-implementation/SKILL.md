...
Reference every task by its Jira ID as the default identifier — in subagent prompts, branch names, worktree names, and status reports. Add the PR number alongside it only where that helps navigation (e.g. the dependency tree, or a status update once a PR exists); don't use the PR number as the task's primary reference.
